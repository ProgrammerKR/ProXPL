...
// ==============================================================================
// 1. Language Knowledge Base & Standard Library Metadata
// ==============================================================================
...
interface DocEntry {
	...
	doc: string;
	example?: string;
	category: 'keyword' | 'builtin' | 'stdlib' | 'type' | 'pillar';
}

const LANGUAGE_DOCS: { [name: string]: DocEntry } = {
	// --- Keywords & Paradigms ---
	'func': {
...
connection.onCompletion(
	(_textDocumentPosition: TextDocumentPositionParams): CompletionItem[] => {
		...
		for (const [key, entry] of Object.entries(LANGUAGE_DOCS)) {
			...
			if (entry.category === 'builtin') kind = CompletionItemKind.Function;
			if (entry.category === 'pillar') kind = CompletionItemKind.Event;
			if (entry.category === 'type') kind = CompletionItemKind.TypeParameter;
