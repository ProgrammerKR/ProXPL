...
// String list for parameters, module names, etc.
typedef struct {
  char **items;
  int count;
  int capacity;
...
// Key-value pair for dictionary expressions
typedef struct {
  Expr *key;
  Expr *value;
} DictPair;
...
typedef struct {
  ...
  StmtList *methods;        // List of FuncDeclStmt
} ClassDeclStmt;
...
typedef struct {
  ...
  StmtList *finally_block; // Can be NULL
} TryCatchStmt;

// --- Main Statement Struct ---
struct Stmt {
  StmtType type;
  int line;
  int column;
  union {
    ...
    ClassDeclStmt class_decl;
    UseDeclStmt use_decl;
    IfStmt if_stmt;
