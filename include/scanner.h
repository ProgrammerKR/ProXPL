// --------------------------------------------------
//   Project: ProX Programming Language (ProXPL)
//   Author:  ProgrammerKR
//   Created: 2025-12-16
//   Copyright © 2025. ProXentix India Pvt. Ltd.  All rights reserved.

#ifndef PROX_SCANNER_H
#define PROX_SCANNER_H

typedef enum {
  // Single-character tokens
  TOKEN_LEFT_PAREN,
  TOKEN_RIGHT_PAREN,
  TOKEN_LEFT_BRACE,
  TOKEN_RIGHT_BRACE,
  TOKEN_LEFT_BRACKET,
  TOKEN_RIGHT_BRACKET,
  TOKEN_COMMA,
  TOKEN_DOT,
  TOKEN_MINUS,
  TOKEN_PLUS,
  TOKEN_SEMICOLON,
  TOKEN_SLASH,
  TOKEN_STAR,
  TOKEN_PERCENT,
  TOKEN_QUESTION,
  TOKEN_COLON,
  TOKEN_BANG,
  TOKEN_TILDE,
  TOKEN_AMPERSAND,
  TOKEN_PIPE,
  TOKEN_CARET,

  // One or two character tokens
  TOKEN_BANG_EQUAL,
  TOKEN_EQUAL,
  TOKEN_EQUAL_EQUAL,
  TOKEN_GREATER,
  TOKEN_GREATER_EQUAL,
  TOKEN_GREATER_GREATER,
  TOKEN_LESS,
  TOKEN_LESS_EQUAL,
  TOKEN_LESS_LESS,
  TOKEN_PLUS_PLUS,
  TOKEN_MINUS_MINUS,
  TOKEN_PLUS_EQUAL,
  TOKEN_MINUS_EQUAL,
  TOKEN_STAR_EQUAL,
  TOKEN_SLASH_EQUAL,
  TOKEN_PERCENT_EQUAL,
  TOKEN_AMPERSAND_EQUAL,
  TOKEN_PIPE_EQUAL,
  TOKEN_CARET_EQUAL,
  TOKEN_AMPERSAND_AMPERSAND,
  TOKEN_PIPE_PIPE,
  TOKEN_DOT_DOT,
  TOKEN_QUESTION_DOT,
  TOKEN_QUESTION_QUESTION,
  TOKEN_ARROW,
  TOKEN_STAR_STAR,

  // Three character tokens
  TOKEN_STAR_STAR_EQUAL,
  TOKEN_LESS_LESS_EQUAL,
  TOKEN_GREATER_GREATER_EQUAL,

  // Literals
  TOKEN_IDENTIFIER,
  TOKEN_STRING,
  TOKEN_NUMBER,

  // Keywords
  TOKEN_AND,
  TOKEN_CLASS,
  TOKEN_ELSE,
  TOKEN_FALSE,
  TOKEN_FOR,
  TOKEN_FUNC,
  TOKEN_IF,
  TOKEN_NULL,
  TOKEN_OR,
  TOKEN_PRINT,
  TOKEN_RETURN,
  TOKEN_SUPER,
  TOKEN_THIS,
  TOKEN_TRUE,
  TOKEN_LET,
  TOKEN_CONST,
  TOKEN_WHILE,
  TOKEN_BREAK,
  TOKEN_CONTINUE,
  TOKEN_SWITCH,
  TOKEN_CASE,
  TOKEN_DEFAULT,
  TOKEN_TRY,
  TOKEN_CATCH,
  TOKEN_FINALLY,
  TOKEN_THROW,
  TOKEN_ASYNC,
  TOKEN_AWAIT,
  TOKEN_USE,
  TOKEN_EXPORT,
  TOKEN_FROM,
  TOKEN_AS,
  TOKEN_IN,
  TOKEN_IS,
  TOKEN_TYPEOF,
  TOKEN_EXTENDS,
  TOKEN_EXTERN,
  TOKEN_IMPLEMENTS,
  TOKEN_INTERFACE,
  TOKEN_ENUM,
  TOKEN_STRUCT,
  TOKEN_STATIC,
  TOKEN_PUBLIC,
  TOKEN_PRIVATE,
  TOKEN_PROTECTED,
  TOKEN_ABSTRACT,
  TOKEN_NATIVE,
  TOKEN_NEW,
  TOKEN_VOID,
  TOKEN_DEFER,
  TOKEN_INTENT,
  TOKEN_RESOLVER,
  TOKEN_AT,
  TOKEN_CONTEXT,
  TOKEN_RESILIENT,
  TOKEN_RECOVERY,
  TOKEN_RESTART,
  TOKEN_ROLLBACK,
  TOKEN_ESCALATE,
  TOKEN_TAINTED,
  TOKEN_PURE,
  TOKEN_SANITIZE,
  TOKEN_POLICY,
  TOKEN_TEMPORAL,
  TOKEN_DECAY,
  TOKEN_AFTER,
  TOKEN_DISTRIBUTED,
  TOKEN_NODE,
  TOKEN_REPLICA,
  TOKEN_SYNC,
  TOKEN_MODEL,
  TOKEN_TRAIN,
  TOKEN_PREDICT,
  TOKEN_DATASET,
  TOKEN_QUANTUM,
  TOKEN_QUBIT,
  TOKEN_SUPERPOSE,
  TOKEN_ENTANGLE,
  TOKEN_MEASURE,
  TOKEN_TENSOR,
  TOKEN_MATRIX,
  TOKEN_GPU,
  TOKEN_KERNEL,
  TOKEN_ENCRYPT,
  TOKEN_DECRYPT,
  TOKEN_VERIFY,
  TOKEN_IDENTITY,
  TOKEN_DO,
  TOKEN_MATCH,
  TOKEN_MUTABLE,
  TOKEN_PACKAGE,
  TOKEN_VAR,
  TOKEN_LAYER,
  TOKEN_ACTIVATE,

  // UI/Frontend Keywords
  TOKEN_UI_APP,
  TOKEN_UI_WINDOW,
  TOKEN_UI_CONTAINER,
  TOKEN_UI_BUTTON,
  TOKEN_UI_TEXT, TOKEN_UI_INPUT,
  TOKEN_UI_NAV, TOKEN_UI_HEADER, TOKEN_UI_FOOTER,
  TOKEN_UI_MAIN, TOKEN_UI_ASIDE, TOKEN_UI_SECTION,
  TOKEN_UI_TABLE, TOKEN_UI_THEAD, TOKEN_UI_TBODY, TOKEN_UI_TR, TOKEN_UI_TH, TOKEN_UI_TD,
  TOKEN_UI_VIDEO, TOKEN_UI_AUDIO, TOKEN_UI_CANVAS,
  TOKEN_UI_PRE, TOKEN_UI_CODE,
  TOKEN_UI_ICON, TOKEN_UI_FORM, TOKEN_UI_LABEL, TOKEN_UI_SELECT, TOKEN_UI_OPTION,
  TOKEN_UI_STATE, TOKEN_UI_ACTION, TOKEN_UI_STYLE,

  TOKEN_ERROR,
  TOKEN_EOF
} PxTokenType;

typedef struct {
  PxTokenType type;
  const char *start;
  int length;
  int line;
  int column;
} Token;

typedef struct {
  const char *start;
  const char *current;
  int line;
  int currentColumn;
  int startColumn;
} Scanner;

void initScanner(Scanner *scanner, const char *source);
Token scanToken(Scanner *scanner);

#endif
